          ...
          >
            {gridModel.map((obj) => {
              ...
              if (obj.columnType.toLowerCase() == "date") {
                formattedDate = /^\d{1,2}\/[A-Za-z]{3}\/\d{4}$/.test(
                  item[key]
                )
                  ? item[key]
                  : parseDate(item[key])
              ...
              return (
                <>
                  <td
                    ...
                    style={
                      {
                        textAlign: obj.columnType.toLowerCase() == "string" ? `left` : `right`,
    ...
    <>
      <div className="recordGridContainter" id="printable-area">
        ...
        <Table bordered responsive striped>
          ...
          {
            (showFotter && tableData.length > 0) && <tfoot style={{ width: '100%', backgroundColor: "pink", height: '3rem' }}>
              <tr>
                {
                  header.map((item, index) => {
                    if (index == 0) {
                      ...
                      return <td className="footColor recordAlignRight footText">{totalQtyFoot}</td>
                    } else if (item.name.toLowerCase().includes("amount") || item.name.toLowerCase().includes("value")) {
                      return <td className="footColor recordAlignRight footText">{totalAmountFoot}</td>
                    }
                    else {
